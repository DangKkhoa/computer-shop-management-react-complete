...
const UserCard = ({ ...props }) => {
  ...
  return (
    <motion.div   
      ...
      className="w-64 h-fit group relative bg-white text-sm text-center border border-gray-300 transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg">
      <button 
      ...
      <div className='p-4'>
        <div 
          ...
          style={{ backgroundImage: `url(http://localhost:3000/uploads/users/${props.image})` }}
        ></div>
        ...
        <div className='flex justify-center items-center gap-4'>
          <button 
            className="bg-gradient-to-tl bg-sky-400  px-3 py-2 hover:bg-sky-500 transition-all duration-300 flex items-center font-semibold rounded-md"
            onClick={() => navigate(`/users/${props.id}`)}>
              <Eye className='mr-2'/> Xem
          </button>
          <button 
            className='bg-gradient-to-tr bg-amber-400 px-3 py-2 hover:bg-amber-500 transition-all duration-300 flex items-center text-nowrap font-semibold rounded-md'

...
const SaleHistory = () => {
  ...
  useEffect(() => {
    document.title = 'Lịch sử mua bán';
    const fetchOrders = async () => {
      try {
        const res = await axios.get('http://localhost:3000/api/v1/sale-history', {withCredentials: true});
        if(res.status === 200) {
  ...
  return (
    <>
      ...
      :
      <>
        <Header title="Lịch sử bán hàng"/>
        <main className='mt-6 p-6 w-full'>
          <div className='w-full overflow-auto'>
            <div className='pl-2'>
              <form action="/sale-history/search" method='GET' className='py-2'>
                <input 
                  type="text" 
                  name='q'
                  placeholder='Nhập id đơn hàng...'
                  className='w-96 mr-2 p-2 outline-none ring-1 ring-gray-300 shadow-sm focus:shadow-lg rounded-md transition-all duration-200'/>
                ...
                {orders.map(order => (
                  <tr key={order.id} className='group hover:bg-gray-100 '>
                    <td className='border-b border-gray-300 p-4'>
                      <Link to={`/sale-history/${order.id}`} className='hover:text-blue-500 hover:font-semibold'>{order.id}</Link>
                    </td>
